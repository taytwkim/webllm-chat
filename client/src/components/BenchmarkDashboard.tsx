...
export default function BenchmarkDashboard({
  ...
  const completedComparisons = comparisonResults.filter(
    (r) => r.local !== null || r.remote !== null
  );
  ...
  const avgLocal =
    localResults.length > 0
      ? {
          ttft:
            localResults.reduce((acc, r) => acc + (r.local?.ttftMs || 0), 0) /
            localResults.length,
          tps:
            localResults.reduce(
              (acc, r) => acc + (r.local?.tokensPerSec || 0),
              0
            ) / localResults.length,
          total:
            localResults.reduce(
              (acc, r) => acc + (r.local?.totalTimeMs || 0),
              0
            ) / localResults.length,
        }
      : null;
  ...
  const avgRemote =
    remoteResults.length > 0
      ? {
          ttft:
            remoteResults.reduce((acc, r) => acc + (r.remote?.ttftMs || 0), 0) /
            remoteResults.length,
          tps:
            remoteResults.reduce(
              (acc, r) => acc + (r.remote?.tokensPerSec || 0),
              0
            ) / remoteResults.length,
          total:
            remoteResults.reduce(
              (acc, r) => acc + (r.remote?.totalTimeMs || 0),
              0
            ) / remoteResults.length,
        }
      : null;
        ...
        <div className="p-6 border-b border-gray-200 flex justify-between items-center bg-gray-50">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Compare Local vs Remote inference performance side-by-side
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="space-y-4">
              ...
              {comparisonResults.map((result, idx) => {
                const isCurrentPrompt =
                  isComparing && idx === progress.current - 1;
                const isPending =
                  result.local === null &&
                  result.remote === null &&
                  !result.localError &&
                ...
                const hasResponses =
                  result.localResponse !== null ||
                  result.remoteResponse !== null;
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      <MetricCell
                        localValue={
                          result.local ? result.local.totalTimeMs / 1000 : null
                        }
                        remoteValue={
                          result.remote
                            ? result.remote.totalTimeMs / 1000
                            : null
                        }
                        lowerIsBetter={true}
                        format={(v) => v.toFixed(2)}
          ...
          {comparisonResults.length === 0 && !isComparing && (
            <div className="text-center py-16">
              ...
              <p className="text-gray-500 max-w-md mx-auto">
                Click "Run Comparison" to test the same prompts on both Local
                and Remote models, and see a live side-by-side comparison of
                their performance.
              </p>
            </div>
          )}
            ...
            <div className="border rounded-lg overflow-hidden">
              <div className="bg-gray-50 px-4 py-2 border-b">
                <h3 className="text-sm font-semibold text-gray-600">
                  Historical Results ({results.length})
                </h3>
              </div>
              <div className="divide-y divide-gray-100">
                {results.slice(-10).map((r, i) => {
                  ...
                  const promptText = (r as any)?.promptText ?? "";
                  const responseText = (r as any)?.response ?? "";
                    ...
                    <div key={i}>
                      ...
                      >
                        ...
                        <div className="text-right font-mono text-sm">
                          {r.metrics.ttftMs.toFixed(0)}
                        </div>
                        <div className="text-right font-mono text-sm">
                          {r.metrics.tokensPerSec.toFixed(1)}
                        </div>
                        <div className="text-right font-mono text-sm">
                          {(r.metrics.totalTimeMs / 1000).toFixed(2)}
                        </div>
                        <div
                          className="text-xs text-gray-500 truncate"
                          title={promptText || ""}
                        >
                          {promptPreview || "(no prompt saved)"}
                        </div>
                      </div>
                      {isExpanded && (
